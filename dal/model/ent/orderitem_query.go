...
func (_q *OrderItemQuery) QueryMenu() *MenuQuery {
...
func (_q *OrderItemQuery) WithMenu(opts ...func(*MenuQuery)) *OrderItemQuery {
	query := (&MenuClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if query := _q.withMenu; query != nil {
		if err := _q.loadMenu(ctx, query, nodes, nil,
			func(n *OrderItem, e *Menu) { n.Edges.Menu = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
