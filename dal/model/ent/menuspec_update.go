// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"errors"
	"fmt"
	"time"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/solikewind/happyeat/dal/model/ent/categoryspec"
	"github.com/solikewind/happyeat/dal/model/ent/menu"
	"github.com/solikewind/happyeat/dal/model/ent/menuspec"
	"github.com/solikewind/happyeat/dal/model/ent/predicate"
	"github.com/solikewind/happyeat/dal/model/ent/specitem"
)

// MenuSpecUpdate is the builder for updating MenuSpec entities.
type MenuSpecUpdate struct {
	config
	hooks    []Hook
	mutation *MenuSpecMutation
}

// Where appends a list predicates to the MenuSpecUpdate builder.
func (_u *MenuSpecUpdate) Where(ps ...predicate.MenuSpec) *MenuSpecUpdate {
	_u.mutation.Where(ps...)
	return _u
}

// SetUpdatedAt sets the "updated_at" field.
func (_u *MenuSpecUpdate) SetUpdatedAt(v time.Time) *MenuSpecUpdate {
	_u.mutation.SetUpdatedAt(v)
	return _u
}

// SetDeleteTs sets the "delete_ts" field.
func (_u *MenuSpecUpdate) SetDeleteTs(v int64) *MenuSpecUpdate {
	_u.mutation.ResetDeleteTs()
	_u.mutation.SetDeleteTs(v)
	return _u
}

// SetNillableDeleteTs sets the "delete_ts" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillableDeleteTs(v *int64) *MenuSpecUpdate {
	if v != nil {
		_u.SetDeleteTs(*v)
	}
	return _u
}

// AddDeleteTs adds value to the "delete_ts" field.
func (_u *MenuSpecUpdate) AddDeleteTs(v int64) *MenuSpecUpdate {
	_u.mutation.AddDeleteTs(v)
	return _u
}

// SetMenuID sets the "menu_id" field.
func (_u *MenuSpecUpdate) SetMenuID(v uint64) *MenuSpecUpdate {
	_u.mutation.SetMenuID(v)
	return _u
}

// SetNillableMenuID sets the "menu_id" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillableMenuID(v *uint64) *MenuSpecUpdate {
	if v != nil {
		_u.SetMenuID(*v)
	}
	return _u
}

// SetSpecItemID sets the "spec_item_id" field.
func (_u *MenuSpecUpdate) SetSpecItemID(v uint64) *MenuSpecUpdate {
	_u.mutation.SetSpecItemID(v)
	return _u
}

// SetNillableSpecItemID sets the "spec_item_id" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillableSpecItemID(v *uint64) *MenuSpecUpdate {
	if v != nil {
		_u.SetSpecItemID(*v)
	}
	return _u
}

// ClearSpecItemID clears the value of the "spec_item_id" field.
func (_u *MenuSpecUpdate) ClearSpecItemID() *MenuSpecUpdate {
	_u.mutation.ClearSpecItemID()
	return _u
}

// SetCategorySpecID sets the "category_spec_id" field.
func (_u *MenuSpecUpdate) SetCategorySpecID(v uint64) *MenuSpecUpdate {
	_u.mutation.SetCategorySpecID(v)
	return _u
}

// SetNillableCategorySpecID sets the "category_spec_id" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillableCategorySpecID(v *uint64) *MenuSpecUpdate {
	if v != nil {
		_u.SetCategorySpecID(*v)
	}
	return _u
}

// ClearCategorySpecID clears the value of the "category_spec_id" field.
func (_u *MenuSpecUpdate) ClearCategorySpecID() *MenuSpecUpdate {
	_u.mutation.ClearCategorySpecID()
	return _u
}

// SetPriceDelta sets the "price_delta" field.
func (_u *MenuSpecUpdate) SetPriceDelta(v int64) *MenuSpecUpdate {
	_u.mutation.ResetPriceDelta()
	_u.mutation.SetPriceDelta(v)
	return _u
}

// SetNillablePriceDelta sets the "price_delta" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillablePriceDelta(v *int64) *MenuSpecUpdate {
	if v != nil {
		_u.SetPriceDelta(*v)
	}
	return _u
}

// AddPriceDelta adds value to the "price_delta" field.
func (_u *MenuSpecUpdate) AddPriceDelta(v int64) *MenuSpecUpdate {
	_u.mutation.AddPriceDelta(v)
	return _u
}

// SetSort sets the "sort" field.
func (_u *MenuSpecUpdate) SetSort(v uint32) *MenuSpecUpdate {
	_u.mutation.ResetSort()
	_u.mutation.SetSort(v)
	return _u
}

// SetNillableSort sets the "sort" field if the given value is not nil.
func (_u *MenuSpecUpdate) SetNillableSort(v *uint32) *MenuSpecUpdate {
	if v != nil {
		_u.SetSort(*v)
	}
	return _u
}

// AddSort adds value to the "sort" field.
func (_u *MenuSpecUpdate) AddSort(v int32) *MenuSpecUpdate {
	_u.mutation.AddSort(v)
	return _u
}

// SetMenu sets the "menu" edge to the Menu entity.
func (_u *MenuSpecUpdate) SetMenu(v *Menu) *MenuSpecUpdate {
	return _u.SetMenuID(v.ID)
}

// SetCategorySpec sets the "category_spec" edge to the CategorySpec entity.
func (_u *MenuSpecUpdate) SetCategorySpec(v *CategorySpec) *MenuSpecUpdate {
	return _u.SetCategorySpecID(v.ID)
}

// SetSpecItem sets the "spec_item" edge to the SpecItem entity.
func (_u *MenuSpecUpdate) SetSpecItem(v *SpecItem) *MenuSpecUpdate {
	return _u.SetSpecItemID(v.ID)
}

// Mutation returns the MenuSpecMutation object of the builder.
func (_u *MenuSpecUpdate) Mutation() *MenuSpecMutation {
	return _u.mutation
}

// ClearMenu clears the "menu" edge to the Menu entity.
func (_u *MenuSpecUpdate) ClearMenu() *MenuSpecUpdate {
	_u.mutation.ClearMenu()
	return _u
}

// ClearCategorySpec clears the "category_spec" edge to the CategorySpec entity.
func (_u *MenuSpecUpdate) ClearCategorySpec() *MenuSpecUpdate {
	_u.mutation.ClearCategorySpec()
	return _u
}

// ClearSpecItem clears the "spec_item" edge to the SpecItem entity.
func (_u *MenuSpecUpdate) ClearSpecItem() *MenuSpecUpdate {
	_u.mutation.ClearSpecItem()
	return _u
}

// Save executes the query and returns the number of nodes affected by the update operation.
func (_u *MenuSpecUpdate) Save(ctx context.Context) (int, error) {
	if err := _u.defaults(); err != nil {
		return 0, err
	}
	return withHooks(ctx, _u.sqlSave, _u.mutation, _u.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (_u *MenuSpecUpdate) SaveX(ctx context.Context) int {
	affected, err := _u.Save(ctx)
	if err != nil {
		panic(err)
	}
	return affected
}

// Exec executes the query.
func (_u *MenuSpecUpdate) Exec(ctx context.Context) error {
	_, err := _u.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_u *MenuSpecUpdate) ExecX(ctx context.Context) {
	if err := _u.Exec(ctx); err != nil {
		panic(err)
	}
}

// defaults sets the default values of the builder before save.
func (_u *MenuSpecUpdate) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if menuspec.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized menuspec.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := menuspec.UpdateDefaultUpdatedAt()
		_u.mutation.SetUpdatedAt(v)
	}
	return nil
}

// check runs all checks and user-defined validators on the builder.
func (_u *MenuSpecUpdate) check() error {
	if _u.mutation.MenuCleared() && len(_u.mutation.MenuIDs()) > 0 {
		return errors.New(`ent: clearing a required unique edge "MenuSpec.menu"`)
	}
	return nil
}

func (_u *MenuSpecUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	if err := _u.check(); err != nil {
		return _node, err
	}
	_spec := sqlgraph.NewUpdateSpec(menuspec.Table, menuspec.Columns, sqlgraph.NewFieldSpec(menuspec.FieldID, field.TypeUint64))
	if ps := _u.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := _u.mutation.UpdatedAt(); ok {
		_spec.SetField(menuspec.FieldUpdatedAt, field.TypeTime, value)
	}
	if value, ok := _u.mutation.DeleteTs(); ok {
		_spec.SetField(menuspec.FieldDeleteTs, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.AddedDeleteTs(); ok {
		_spec.AddField(menuspec.FieldDeleteTs, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.PriceDelta(); ok {
		_spec.SetField(menuspec.FieldPriceDelta, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.AddedPriceDelta(); ok {
		_spec.AddField(menuspec.FieldPriceDelta, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.Sort(); ok {
		_spec.SetField(menuspec.FieldSort, field.TypeUint32, value)
	}
	if value, ok := _u.mutation.AddedSort(); ok {
		_spec.AddField(menuspec.FieldSort, field.TypeUint32, value)
	}
	if _u.mutation.MenuCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.MenuTable,
			Columns: []string{menuspec.MenuColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.MenuIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.MenuTable,
			Columns: []string{menuspec.MenuColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if _u.mutation.CategorySpecCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.CategorySpecTable,
			Columns: []string{menuspec.CategorySpecColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(categoryspec.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.CategorySpecIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.CategorySpecTable,
			Columns: []string{menuspec.CategorySpecColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(categoryspec.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if _u.mutation.SpecItemCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.SpecItemTable,
			Columns: []string{menuspec.SpecItemColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(specitem.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.SpecItemIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.SpecItemTable,
			Columns: []string{menuspec.SpecItemColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(specitem.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if _node, err = sqlgraph.UpdateNodes(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{menuspec.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return 0, err
	}
	_u.mutation.done = true
	return _node, nil
}

// MenuSpecUpdateOne is the builder for updating a single MenuSpec entity.
type MenuSpecUpdateOne struct {
	config
	fields   []string
	hooks    []Hook
	mutation *MenuSpecMutation
}

// SetUpdatedAt sets the "updated_at" field.
func (_u *MenuSpecUpdateOne) SetUpdatedAt(v time.Time) *MenuSpecUpdateOne {
	_u.mutation.SetUpdatedAt(v)
	return _u
}

// SetDeleteTs sets the "delete_ts" field.
func (_u *MenuSpecUpdateOne) SetDeleteTs(v int64) *MenuSpecUpdateOne {
	_u.mutation.ResetDeleteTs()
	_u.mutation.SetDeleteTs(v)
	return _u
}

// SetNillableDeleteTs sets the "delete_ts" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillableDeleteTs(v *int64) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetDeleteTs(*v)
	}
	return _u
}

// AddDeleteTs adds value to the "delete_ts" field.
func (_u *MenuSpecUpdateOne) AddDeleteTs(v int64) *MenuSpecUpdateOne {
	_u.mutation.AddDeleteTs(v)
	return _u
}

// SetMenuID sets the "menu_id" field.
func (_u *MenuSpecUpdateOne) SetMenuID(v uint64) *MenuSpecUpdateOne {
	_u.mutation.SetMenuID(v)
	return _u
}

// SetNillableMenuID sets the "menu_id" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillableMenuID(v *uint64) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetMenuID(*v)
	}
	return _u
}

// SetSpecItemID sets the "spec_item_id" field.
func (_u *MenuSpecUpdateOne) SetSpecItemID(v uint64) *MenuSpecUpdateOne {
	_u.mutation.SetSpecItemID(v)
	return _u
}

// SetNillableSpecItemID sets the "spec_item_id" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillableSpecItemID(v *uint64) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetSpecItemID(*v)
	}
	return _u
}

// ClearSpecItemID clears the value of the "spec_item_id" field.
func (_u *MenuSpecUpdateOne) ClearSpecItemID() *MenuSpecUpdateOne {
	_u.mutation.ClearSpecItemID()
	return _u
}

// SetCategorySpecID sets the "category_spec_id" field.
func (_u *MenuSpecUpdateOne) SetCategorySpecID(v uint64) *MenuSpecUpdateOne {
	_u.mutation.SetCategorySpecID(v)
	return _u
}

// SetNillableCategorySpecID sets the "category_spec_id" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillableCategorySpecID(v *uint64) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetCategorySpecID(*v)
	}
	return _u
}

// ClearCategorySpecID clears the value of the "category_spec_id" field.
func (_u *MenuSpecUpdateOne) ClearCategorySpecID() *MenuSpecUpdateOne {
	_u.mutation.ClearCategorySpecID()
	return _u
}

// SetPriceDelta sets the "price_delta" field.
func (_u *MenuSpecUpdateOne) SetPriceDelta(v int64) *MenuSpecUpdateOne {
	_u.mutation.ResetPriceDelta()
	_u.mutation.SetPriceDelta(v)
	return _u
}

// SetNillablePriceDelta sets the "price_delta" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillablePriceDelta(v *int64) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetPriceDelta(*v)
	}
	return _u
}

// AddPriceDelta adds value to the "price_delta" field.
func (_u *MenuSpecUpdateOne) AddPriceDelta(v int64) *MenuSpecUpdateOne {
	_u.mutation.AddPriceDelta(v)
	return _u
}

// SetSort sets the "sort" field.
func (_u *MenuSpecUpdateOne) SetSort(v uint32) *MenuSpecUpdateOne {
	_u.mutation.ResetSort()
	_u.mutation.SetSort(v)
	return _u
}

// SetNillableSort sets the "sort" field if the given value is not nil.
func (_u *MenuSpecUpdateOne) SetNillableSort(v *uint32) *MenuSpecUpdateOne {
	if v != nil {
		_u.SetSort(*v)
	}
	return _u
}

// AddSort adds value to the "sort" field.
func (_u *MenuSpecUpdateOne) AddSort(v int32) *MenuSpecUpdateOne {
	_u.mutation.AddSort(v)
	return _u
}

// SetMenu sets the "menu" edge to the Menu entity.
func (_u *MenuSpecUpdateOne) SetMenu(v *Menu) *MenuSpecUpdateOne {
	return _u.SetMenuID(v.ID)
}

// SetCategorySpec sets the "category_spec" edge to the CategorySpec entity.
func (_u *MenuSpecUpdateOne) SetCategorySpec(v *CategorySpec) *MenuSpecUpdateOne {
	return _u.SetCategorySpecID(v.ID)
}

// SetSpecItem sets the "spec_item" edge to the SpecItem entity.
func (_u *MenuSpecUpdateOne) SetSpecItem(v *SpecItem) *MenuSpecUpdateOne {
	return _u.SetSpecItemID(v.ID)
}

// Mutation returns the MenuSpecMutation object of the builder.
func (_u *MenuSpecUpdateOne) Mutation() *MenuSpecMutation {
	return _u.mutation
}

// ClearMenu clears the "menu" edge to the Menu entity.
func (_u *MenuSpecUpdateOne) ClearMenu() *MenuSpecUpdateOne {
	_u.mutation.ClearMenu()
	return _u
}

// ClearCategorySpec clears the "category_spec" edge to the CategorySpec entity.
func (_u *MenuSpecUpdateOne) ClearCategorySpec() *MenuSpecUpdateOne {
	_u.mutation.ClearCategorySpec()
	return _u
}

// ClearSpecItem clears the "spec_item" edge to the SpecItem entity.
func (_u *MenuSpecUpdateOne) ClearSpecItem() *MenuSpecUpdateOne {
	_u.mutation.ClearSpecItem()
	return _u
}

// Where appends a list predicates to the MenuSpecUpdate builder.
func (_u *MenuSpecUpdateOne) Where(ps ...predicate.MenuSpec) *MenuSpecUpdateOne {
	_u.mutation.Where(ps...)
	return _u
}

// Select allows selecting one or more fields (columns) of the returned entity.
// The default is selecting all fields defined in the entity schema.
func (_u *MenuSpecUpdateOne) Select(field string, fields ...string) *MenuSpecUpdateOne {
	_u.fields = append([]string{field}, fields...)
	return _u
}

// Save executes the query and returns the updated MenuSpec entity.
func (_u *MenuSpecUpdateOne) Save(ctx context.Context) (*MenuSpec, error) {
	if err := _u.defaults(); err != nil {
		return nil, err
	}
	return withHooks(ctx, _u.sqlSave, _u.mutation, _u.hooks)
}

// SaveX is like Save, but panics if an error occurs.
func (_u *MenuSpecUpdateOne) SaveX(ctx context.Context) *MenuSpec {
	node, err := _u.Save(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// Exec executes the query on the entity.
func (_u *MenuSpecUpdateOne) Exec(ctx context.Context) error {
	_, err := _u.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_u *MenuSpecUpdateOne) ExecX(ctx context.Context) {
	if err := _u.Exec(ctx); err != nil {
		panic(err)
	}
}

// defaults sets the default values of the builder before save.
func (_u *MenuSpecUpdateOne) defaults() error {
	if _, ok := _u.mutation.UpdatedAt(); !ok {
		if menuspec.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized menuspec.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := menuspec.UpdateDefaultUpdatedAt()
		_u.mutation.SetUpdatedAt(v)
	}
	return nil
}

// check runs all checks and user-defined validators on the builder.
func (_u *MenuSpecUpdateOne) check() error {
	if _u.mutation.MenuCleared() && len(_u.mutation.MenuIDs()) > 0 {
		return errors.New(`ent: clearing a required unique edge "MenuSpec.menu"`)
	}
	return nil
}

func (_u *MenuSpecUpdateOne) sqlSave(ctx context.Context) (_node *MenuSpec, err error) {
	if err := _u.check(); err != nil {
		return _node, err
	}
	_spec := sqlgraph.NewUpdateSpec(menuspec.Table, menuspec.Columns, sqlgraph.NewFieldSpec(menuspec.FieldID, field.TypeUint64))
	id, ok := _u.mutation.ID()
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "MenuSpec.id" for update`)}
	}
	_spec.Node.ID.Value = id
	if fields := _u.fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, menuspec.FieldID)
		for _, f := range fields {
			if !menuspec.ValidColumn(f) {
				return nil, &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
			}
			if f != menuspec.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, f)
			}
		}
	}
	if ps := _u.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if value, ok := _u.mutation.UpdatedAt(); ok {
		_spec.SetField(menuspec.FieldUpdatedAt, field.TypeTime, value)
	}
	if value, ok := _u.mutation.DeleteTs(); ok {
		_spec.SetField(menuspec.FieldDeleteTs, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.AddedDeleteTs(); ok {
		_spec.AddField(menuspec.FieldDeleteTs, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.PriceDelta(); ok {
		_spec.SetField(menuspec.FieldPriceDelta, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.AddedPriceDelta(); ok {
		_spec.AddField(menuspec.FieldPriceDelta, field.TypeInt64, value)
	}
	if value, ok := _u.mutation.Sort(); ok {
		_spec.SetField(menuspec.FieldSort, field.TypeUint32, value)
	}
	if value, ok := _u.mutation.AddedSort(); ok {
		_spec.AddField(menuspec.FieldSort, field.TypeUint32, value)
	}
	if _u.mutation.MenuCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.MenuTable,
			Columns: []string{menuspec.MenuColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.MenuIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.MenuTable,
			Columns: []string{menuspec.MenuColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(menu.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if _u.mutation.CategorySpecCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.CategorySpecTable,
			Columns: []string{menuspec.CategorySpecColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(categoryspec.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.CategorySpecIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.CategorySpecTable,
			Columns: []string{menuspec.CategorySpecColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(categoryspec.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	if _u.mutation.SpecItemCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.SpecItemTable,
			Columns: []string{menuspec.SpecItemColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(specitem.FieldID, field.TypeUint64),
			},
		}
		_spec.Edges.Clear = append(_spec.Edges.Clear, edge)
	}
	if nodes := _u.mutation.SpecItemIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   menuspec.SpecItemTable,
			Columns: []string{menuspec.SpecItemColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(specitem.FieldID, field.TypeUint64),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_spec.Edges.Add = append(_spec.Edges.Add, edge)
	}
	_node = &MenuSpec{config: _u.config}
	_spec.Assign = _node.assignValues
	_spec.ScanValues = _node.scanValues
	if err = sqlgraph.UpdateNode(ctx, _u.driver, _spec); err != nil {
		if _, ok := err.(*sqlgraph.NotFoundError); ok {
			err = &NotFoundError{menuspec.Label}
		} else if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	_u.mutation.done = true
	return _node, nil
}
