...
// QueryMenuCovers chains the current query on the "menu_covers" edge.
func (_q *ObjectQuery) QueryMenuCovers() *MenuQuery {
...
func (_q *ObjectQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ObjectQuery) FirstIDX(ctx context.Context) uint64 {
...
func (_q *ObjectQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (_q *ObjectQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (_q *ObjectQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (_q *ObjectQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ObjectQuery) Clone() *ObjectQuery {
	...
	return &ObjectQuery{
		...
		withMenuCovers: _q.withMenuCovers.Clone(),
...
// WithMenuCovers tells the query-builder to eager-load the nodes that are connected to
// the "menu_covers" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ObjectQuery) WithMenuCovers(opts ...func(*MenuQuery)) *ObjectQuery {
	query := (&MenuClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMenuCovers = query
	return _q
}
...
func (_q *ObjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Object, error) {
	...
	if query := _q.withMenuCovers; query != nil {
		if err := _q.loadMenuCovers(ctx, query, nodes,
			func(n *Object) { n.Edges.MenuCovers = []*Menu{} },
			func(n *Object, e *Menu) { n.Edges.MenuCovers = append(n.Edges.MenuCovers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ObjectQuery) loadMenuCovers(ctx context.Context, query *MenuQuery, nodes []*Object, init func(*Object), assign func(*Object, *Menu)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Object)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(menu.FieldObjectID)
	}
	query.Where(predicate.Menu(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(object.MenuCoversColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.ObjectID
		if fk == nil {
			return fmt.Errorf(`foreign-key "object_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *ObjectQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(object.Table, object.Columns, sqlgraph.NewFieldSpec(object.FieldID, field.TypeUint64))
	_spec.From = _q.sql
