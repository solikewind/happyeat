...
- IAM 表与其它业务表一样，由 **`make migrate`（ent `Schema.Create`）** 创建；建表后再启动服务，`RbacStore` 会在首次启动时写入种子角色/权限数据。
- 若不用 ent 迁移、仅想手工建 IAM 表，可参考 `dal/casbin/init_rbac.sql`（与当前 ent 定义可能不完全一致，优先以 migrate 为准）。
